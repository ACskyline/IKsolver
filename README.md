...
## Pseudo Inverse
In my opinion, pseudo inverse is the most theoretical way to solve IK. It literally take the increment of position and using the inverse of the Jacobean matrix to calculate the increment of orientation. When acquiring the inverse of Jacobean matrix, it is not guaranteed that the matrix is square(non-square matrix is not invertible), which is the reason why pseudo inverse is used. Also, in practical, damped pseudo inverse is used in case of singular matrix and null space operator is used to keep the posture close to the original posture(which I didn't use as you can see from the gif below XD).
